...
taxonomic classifications of Jovian and CAT, to compare the
output of the two methods.

INPUT:
  -  table of Jovian classified superkingdoms (tab-separated)
...
OUTPUT:
  -  grouped stacked bar graph of superkingdom classifications
     by Jovian and CAT, quantified as number of reads (to
...
def draw_stacked_bars(jovian_df, cat_df):
    """
    Draw grouped stacked bar charts
    Input:
        dataframe with Jovian's quantified classifications,
        dataframe with CAT's quantified classifications
    Output:
    ...
    for sample in SAMPLES:
        for tool in [ "Jovian", "CAT" ]:
            factors.append((sample, tool))
            if tool == "Jovian":
                arch.append(jovian_df[jovian_df["Sample_name"] == sample]["Archaea"].iloc[0])
                bact.append(jovian_df[jovian_df["Sample_name"] == sample]["Bacteria"].iloc[0])
                euk.append(jovian_df[jovian_df["Sample_name"] == sample]["Eukaryota"].iloc[0])
                vir.append(jovian_df[jovian_df["Sample_name"] == sample]["Viruses"].iloc[0])
                uncl.append(jovian_df[jovian_df["Sample_name"] == sample]["not classified"].iloc[0])
            elif tool == "CAT":
...
def main():
    ...
    """
    ## 1. import and select Jovian input data:
    jovian_file = snakemake.input["jovian"]
    jovian_df = pd.read_csv(jovian_file)
    jovian_df.rename(columns={ "Unclassified" : "not classified",
                           "Sample" : "Sample_name" }, inplace=True)
    jovian_df = jovian_df.loc[ jovian_df["Sample_name"].isin(SAMPLES) ]
    ...
    draw_stacked_bars(jovian_df, cat_df)
